...
printf 'docker cp + restart web-terminal-kiro-dev\n'
...
printf 'waiting for web-terminal-kiro-dev health (timeout %ss)\n' "$DEPLOY_TIMEOUT"
...
while [ "$(date +%s)" -lt "$deadline" ]; do
  ...
  state=$(timeout --signal=TERM --kill-after=5s 15s ssh "${SSH_OPTS[@]}" "$HOST" "sudo docker inspect -f '{{.State.Running}} {{.RestartCount}}' web-terminal-kiro-dev" 2>/dev/null || printf 'unknown unknown\n')
  running=${state%% *}
  restarts=${state##* }
...
printf 'web-terminal-kiro-dev health: %s  (UI: http://%s:9849/)\n' "$code" "$PROBE_HOST"
[ "$code" = "200" ] || {
