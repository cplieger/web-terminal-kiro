// CDP live-verify for bug 2 (sleep/wake loses content), vterm/docs/REBUILD.md
// brick 6. Drives a REAL disconnect→reconnect cycle against a vibecli instance
// whose PTY is the deterministic emitter (scripts/emit-fixture.sh), which keeps
// printing lines server-side the whole time. While the socket is severed the
// emitter accrues lines the client never saw; on reconnect the client resumes
// by absolute index (haveThrough) and the server replays exactly the missed
// lines. The pass criteria encode the bug's absence:
//
...
//   - no absolute index appears twice (no duplicate rows = bug 5 stays fixed)
...
// clears the flag and fires `pageshow` (the iOS-wake signal vibecli binds
// reconnectNow to). Zero deps (Node 22 global WebSocket + fetch).
//
// Usage: node scripts/cdp-bug2.cjs
...
(async () => {
  ...
  // app document loads, then navigate to vibecli.
  ...
  // --- recover: unblock + the iOS-wake signal vibecli reconnects on ---
  ...
  const checks = {
    ...
    "no duplicate rows after recovery (bug 5 stays fixed)": after.dupCount === 0,
